<template>
  <div>
    ...
    <div>
      <div>
        <span class="_12avHmKjidVRHTuf5Qbd4K"><svg viewBox="0 0 1024 1024" width="18px" height="18px">
            <path
      ...
      <div>
          <!-- <span v-if="showClose" @click="$emit('closeComment', false)">收起</span> -->
          <!-- <span v-if="showClose" @click="$emit('input', false)">收起</span> -->
          <span v-if="showClose" @click="$emit('update:showComment', false)">收起</span>
          <button :disabled="!enable" @click="publishComment">发布</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    placeholder() {
      if (this.replyComment) {
        return `${this.$t("comment.replyTip")} ${this.replyComment.name}`;
      } else {
        return this.$t("comment.defaultTip");
      }
    },
    showClose() {
  ...
  methods: {
      async publishComment(){
          ...
          if (this.replyComment){
              options = {
                html: this.replyComment.html,
                userAgent: this.replyComment.userAgent,
                pass: false,
                parentCommentId: this.replyComment.id,
                replyUserEmail: "2870856034@qq.com",
                replyUserName: "123"
              };
          }
          let result = await publishComment(this.content, this.hostId, options);
          console.log('result...', result);
          if (result){
              Message.success('评论发表成功，正在审核中!')
          }else{
